...
%% Create GLM Model
% mark pps with insufficent misses
bad_pp = false(n_subj,1);
...
for subj = 1 : n_subj
    ...
    mkdir([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/'])
    
    % delete old SPM file if it exists
    if exist([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/SPM.mat'],'file'); delete([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/SPM.mat']); end
    ...
    % check that there are enough misses for RSA
    blk1 = all(sum(~isnan(events_onset(1:12,5:8))) >= 1);
    blk2 = all(sum(~isnan(events_onset(13:24,5:8))) >= 1);
    if ~blk1 || ~blk2
        bad_pp(subj) = true;
        warning('insufficent misses for sub-%02.0f...\n',subj)
        continue
    end
    ...
    else; save([dir_subj,'/rsa-ers-forgotten/R.mat'],'R') %#ok<*UNRCH>
    end
    
    % get all scans for GLM
    all_scans = get_functional_files([dir_root,'derivatives/',subj_handle,'/'],'ua',copylocal);
    ...
    if copylocal
        ...
        matlabbatch{1}.spm.stats.fmri_spec.dir              = {[dir_subj,'/rsa-ers-forgotten']};
        matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg   = {[dir_subj,'/rsa-ers-forgotten/R.mat']}; 
    end
    ...
    else; matlabbatch{2}.spm.stats.fmri_est.spmmat = {[dir_subj,'/rsa-ers-forgotten/SPM.mat']};
    end
    ...
    if copylocal
        copyfile('C:/tmp_mri/spm/R.mat',[dir_subj,'/rsa-ers-forgotten/R.mat'])
        copyfile('C:/tmp_mri/spm/SPM.mat',[dir_subj,'/rsa-ers-forgotten/SPM.mat'])
        copyfile('C:/tmp_mri/spm/mask.nii',[dir_subj,'/rsa-ers-forgotten/mask.nii'])
        rmdir('C:/tmp_mri/','s'); 
    end
end
...
for subj = 1 : n_subj
    ...
    nii = load_untouch_nii([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/mask.nii']);
    maskImg(1,:) = reshape(nii.img,1,[]);

    % predefine matrix for functional data
    scanVec = zeros((n_volumes-8).*8,numel(nii.img));
    ...
    fprintf('\nSaving full volume...\n')
    mkdir([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/'])
    save([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-fullVolume.mat'],'scanVec')
    ...
    save([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-maskedVolume.mat'],'patterns')
    save([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-mask.mat'],'mask_idx')
...
for subj = 1 : n_subj
    ...
    % load pattern data
    load([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-maskedVolume.mat'],'patterns')
    ...
    % save patterns
    save([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-maskedDemeanedVolume.mat'],'patterns')
...
for subj = 1 : n_subj
    ...
    % load pattern data
    load([dir_subj,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-maskedDemeanedVolume.mat'],'patterns')
    
    % load SPM.mat
    load([dir_subj,'/rsa-ers-forgotten/SPM.mat'],'SPM')
    ...
    % save data
    save([dir_subj,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-FormattedVolume.mat'],'X','Y')
...
for subj = 1 : n_subj
    ...
    % load data
    load([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-FormattedVolume.mat'],'X','Y')
    load([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-mask.mat'],'mask_idx')
    ...
    % save raw output
    save([dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-rawRDM.mat'],'RDM_ldt')
    ...
    % load template nifti
    filename = [dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/mask.nii'];        
    V = load_untouch_nii(filename);

    % change filename, datatype, and image
    V.fileprefix = [dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-searchlight'];
...
for subj = 1 : n_subj
    ...
    matlabbatch{1}.spm.spatial.normalise.write.subj.def         = {[dir_root,'derivatives/',subj_handle,'/anat/y_',subj_handle,'_T1w.nii']};
    matlabbatch{1}.spm.spatial.normalise.write.subj.resample    = {[dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/',subj_handle,'_task-rf_rsa-searchlight.nii,1']};
    ...
    matlabbatch{2}.spm.spatial.smooth.data                      = {[dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/w',subj_handle,'_task-rf_rsa-searchlight.nii,1']};
...
for subj = 1 : n_subj
    ...
    rMapFiles{end+1,1}  = [dir_root,'derivatives/',subj_handle,'/rsa-ers-forgotten/sw',subj_handle,'_task-rf_rsa-searchlight.nii,1']; %#ok<AGROW>
end

% create second-level glms
matlabbatch{1}.spm.stats.factorial_design.dir                       = {[dir_root,'derivatives/group/rsa-ers-forgotten']};
...
% estimate model
matlabbatch{1}.spm.stats.fmri_est.spmmat            = {[dir_root,'derivatives/group/rsa-ers-forgotten/SPM.mat']};
...
% define contrasts
matlabbatch{1}.spm.stats.con.spmmat(1)                  = {[dir_root,'derivatives/group/rsa-ers-forgotten/SPM.mat']};   
...
combine_spm_cluster([dir_root,'derivatives/group/rsa-ers-forgotten/'])

% load SPM details
load([dir_root,'derivatives/group/rsa-ers-forgotten/SPM.mat'],'SPM')

% extract subject values for each cluster
[betas,d] = extract_sample_points([dir_root,'derivatives/group/rsa-ers-forgotten/'],SPM);

% save betas as table
tbl = array2table(betas','VariableNames',{'RightCentral'});
writetable(tbl,[dir_repos,'data/fig1_data/ers_forg_betas.csv'],'Delimiter',',')

% save effect size as table
tbl = array2table(d','VariableNames',{'RightCentral'});
writetable(tbl,[dir_repos,'data/fig1_data/ers_forg_cohensD.csv'],'Delimiter',',')
