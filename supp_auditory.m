...
for subj = 1 : n_subj
    ...
    rMapFiles{subj,1}  = [dir_root,'derivatives/',subj_handle,'/rsa-percept-audio/sw',subj_handle,'_task-percept_rsa-searchlightVisual.nii,1'];
end

% make group directory
mkdir([dir_root,'derivatives/group/rsa-percept-audio/'])

% delete spm if it exists
if exist([dir_root,'derivatives/group/rsa-percept-audio/SPM.mat'],'file'); delete([dir_root,'derivatives/group/rsa-percept-audio/SPM.mat']); end

% create second-level glms
matlabbatch{1}.spm.stats.factorial_design.dir                       = {[dir_root,'derivatives/group/rsa-percept-audio/']};
...
% estimate model
matlabbatch{1}.spm.stats.fmri_est.spmmat            = {[dir_root,'derivatives/group/rsa-percept-audio/SPM.mat']};
...
% define contrasts
matlabbatch{1}.spm.stats.con.spmmat(1)                  = {[dir_root,'derivatives/group/rsa-percept-audio/SPM.mat']};   
...
combine_spm_cluster([dir_root,'derivatives/group/rsa-percept-audio/'])

% load SPM details
load([dir_root,'derivatives/group/rsa-percept-audio/SPM.mat'],'SPM')

% extract subject values for each cluster
[betas,d] = extract_sample_points([dir_root,'derivatives/group/rsa-percept-audio/'],SPM);

% save betas as table
tbl = array2table(betas','VariableNames',{'RightTemp','LeftTemp'});
writetable(tbl,[dir_repos,'data/fig1_data/percept_aud_betas.csv'],'Delimiter',',')

% save effect size as table
tbl = array2table(d','VariableNames',{'RightTemp','LeftTemp'});
writetable(tbl,[dir_repos,'data/fig1_data/percept_aud_cohensD.csv'],'Delimiter',',')
...
for subj = 1 : n_subj
    ...
    % get searchlight images
    rMapFiles{subj,1}  = [dir_root,'derivatives/',subj_handle,'/rsa-ers-auditory/sw',subj_handle,'_task-rf_rsa-searchlight_response.nii,1'];
end

% create second-level glms
matlabbatch{1}.spm.stats.factorial_design.dir                       = {[dir_root,'derivatives/group/rsa-ers-auditory']};
matlabbatch{1}.spm.stats.factorial_design.des.t1.scans              = rMapFiles(:,1);
matlabbatch{1}.spm.stats.factorial_design.cov                       = struct('c', {}, 'cname', {}, 'iCFI', {}, 'iCC', {});
matlabbatch{1}.spm.stats.factorial_design.multi_cov                 = struct('files', {}, 'iCFI', {}, 'iCC', {});
matlabbatch{1}.spm.stats.factorial_design.masking.tm.tm_none        = 1;
...
% estimate model
matlabbatch{1}.spm.stats.fmri_est.spmmat            = {[dir_root,'derivatives/group/rsa-ers-auditory/SPM.mat']};
matlabbatch{1}.spm.stats.fmri_est.write_residuals   = 0;
matlabbatch{1}.spm.stats.fmri_est.method.Classical  = 1;
...
% define contrasts
matlabbatch{1}.spm.stats.con.spmmat(1)                  = {[dir_root,'derivatives/group/rsa-ers-auditory/SPM.mat']};   
matlabbatch{1}.spm.stats.con.delete                     = 0;    
matlabbatch{1}.spm.stats.con.consess{1}.tcon.name       = 'within>between';
matlabbatch{1}.spm.stats.con.consess{1}.tcon.convec     = 1;
matlabbatch{1}.spm.stats.con.consess{1}.tcon.sessrep    = 'none';  
...
%% Extract Metrics for Visualisation
% combine visual cluster and save
combine_spm_cluster([dir_root,'derivatives/group/rsa-ers-auditory/'])

% load SPM details
load([dir_root,'derivatives/group/rsa-ers-auditory/SPM.mat'],'SPM')

% extract subject values for each cluster
[betas,d] = extract_sample_points([dir_root,'derivatives/group/rsa-ers-auditory/'],SPM);

% save betas as table
tbl = array2table(betas','VariableNames',{'LeftFrontal'});
writetable(tbl,[dir_repos,'data/fig1_data/ers_aud_betas.csv'],'Delimiter',',')

% save effect size as table
tbl = array2table(d','VariableNames',{'LeftFrontal'});
writetable(tbl,[dir_repos,'data/fig1_data/ers_aud_cohensD.csv'],'Delimiter',',')
...
for subj = 1 : n_subj
    ...
    % load RDM
    load([dir_subj,'/rsa-correlation-audio/',subj_handle,'_task-percept_rsa-rdm.mat'],'RDM')
...
% save RSA vector
save([dir_root,'derivatives/group/rsa-correlation-audio/group_task-all_fmri-si'],'rsa_vec')
...
dir_subj = [dir_root,'sourcedata/sub-01/eeg/'];
...
% load mask
mask_roi{1} = ft_read_mri([dir_root,'/derivatives/group/rsa-percept-audio/grand_cluster_dilated.nii']);

% interpolate
mask_roi{1} = ft_sourceinterpolate(cfg,mask_roi{1},sourcemodel);
mask_roi{1}.anatomy = mask_roi{1}.anatomy(:);

% add
mask_roi{1}.freq_2_roi = find(mask_roi{1}.anatomy(roi.inside==1)>0);
mask_roi{1}.inside	= mask_roi{1}.anatomy(:) > 0 & roi.inside == 1;
mask_roi{1}.label    = source.label(mask_roi{1}.inside(roi.inside == 1));
mask_roi{1}.pos    	= sourcemodel.pos;
...
if ~skipEEG;tic
    ...
    for subj = 1 : n_subj
        ...
        load(sprintf('%sderivatives/sub-%02.0f/eeg/sub-%02.0f_task-rf_eeg-wavelet.mat',dir_root,subj,subj),'freq')  
        ...
        save(sprintf('%sderivatives/sub-%02.0f/eeg/sub-%02.0f_task-audiocorr_eeg-wavelet.mat',dir_root,subj,subj),'freq')  
    ...
    for subj = 1 : n_subj
        ...
        % get irasa
        freq{1} = get_roi_irasa(source,mask_roi{1},'encoding','auditory');

        % save spectral outputs
        save(sprintf('%sderivatives/sub-%02.0f/eeg/sub-%02.0f_task-audiocorr_eeg-irasa.mat',dir_bids,subj,subj),'freq')  
        fprintf('\nsub-%02.0f complete...\ntotal time elapsed: %1.1f minutes...\nestimated time remaining: %1.1f minutes...\n',subj,round(toc/60,1),round((round(toc/60,1)/subj)*(n_subj-subj),1))
...
% save data
save([dir_root,'derivatives/group/rsa-correlation-audio/group_task-rf_comb-freq.mat'],'grand_freq'); 
