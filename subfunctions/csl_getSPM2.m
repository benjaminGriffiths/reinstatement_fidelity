function [SPM,xSPM] = csl_getSPM2(xSPM)
% Adapted version of spm_getSPM.m with few changes to handle the xSPM structure.
% allowing automatic processing.      
% computes a specified and thresholded SPM/PPM following parameter estimation
% FORMAT [SPM,xSPM] = csl_getSPM2;
%
...
%-----------------------------------------------------------------------
%%%COMMENTED OUT BY JAG--REMOVES FINAL DIRECTORY
%swd    = spm_str_manip(spm_get(1,'SPM.mat','Select SPM.mat'),'H');

% try
% 	swd = spm_str_manip(xSPM.swd,'H');
% catch	
% 	swd    = spm_str_manip(spm_get(1,'SPM.mat','Select SPM.mat'),'H');
% 
% end
%%%USE THE DIRECTORY SPECIFIED IN THE BATCH INSTEAD
swd  = xSPM.swd
%%%
...
%-----------------------------------------------------------------------
maskCon= 0; %by defualt don't use masking. 
try
	Im = xSPM.Im;
	if ~isempty(Im) maskCon=1; end 
catch
...
if STAT ~= 'P'
   ...
   try
	...
	Mcp = spm_input('p value adjustment to control','+1','b',str,[],1);
    end
    switch Mcp
