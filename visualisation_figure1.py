...
# define labels
labels = {'title':'',
          'ylabel':'Encoding-Retrieval Similarity Index (z)',
...
# define colour scheme
colour = sns.color_palette("Reds",n_colors=7)
colour = [colour[4],(0.7,0.7,0.7)]

# define raincloud data filename
data_fname = wdir + "data/fig1_data/group_task-retrieval_histogram.csv"

# load raincloud data
data_distribution = pandas.read_csv(data_fname,
                       delimiter=",",
                       header=None,
                       names=['Values','Memory'])

data = data_distribution['Values'][data_distribution['Memory']==0]
sns.kdeplot(data, 
            color=colour[1],
            shade=True,
            linewidth=1)

data = data_distribution['Values'][data_distribution['Memory']==1]
sns.kdeplot(data, 
            color=colour[0],
            shade=True,
            linewidth=1)
...
# add horizontal line
ax.axvline(x=0, ymin=-1, ymax=3, color=[0,0,0], linewidth = 1)

# aesthetics
ax.set_ylabel('Kernel Density',fontname='calibri',fontsize=6,labelpad=3,fontweight='light')   # add Y axis label
ax.set_xlabel('Encoding-Retrieval Similarity Index',fontname='calibri',fontsize=6,labelpad=3,fontweight='light')   # add Y axis label
ax.set_xlim([-0.8,0.8])                  # set Y axis range to 0 -> 1
ax.set_ylim([0,2])
...
ax.legend(['Forgot.','Rem.'],frameon=False,fontsize=5,bbox_to_anchor=(0., 1.02, 1., .102), 
          loc=3, ncol=2, mode="expand", borderaxespad=0.)

ax.set_xticks([-0.5,0,0.5])
ax.set_yticks([0,1,2])
ax.set_xticklabels(['-0.5','0','0.5'],fontname='calibri',fontweight='light',fontsize=6)
ax.set_yticklabels(['0','1','2'],fontname='calibri',fontweight='light',fontsize=6)
